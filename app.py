...
try:
    ...
    class SMACrossover(_BaseStrategy):
        pass
    class RSIMeanReversion(_BaseStrategy):
        pass
...
try:
    ...
    class MLStrategy(_BaseStrategy):
        pass
    class ReinforcementStrategy(_BaseStrategy):
        pass
...
if IMPORT_ERRORS:
    st.warning("Mode dégradé activé : certains modules locaux sont introuvables sur l'environnement Streamlit. L'interface reste utilisable avec des objets de démonstration.")
...
with st.sidebar:
    ...
    if st.button("Create bot", use_container_width=True):
        ...
        st.session_state["logs"][bot_id] = [f"[INFO] {datetime.now().strftime('%H:%M:%S')} - Bot created"]
...
def gen_demo_candles():
    df = DataFeed().get_data()
    if isinstance(df, pd.DataFrame) and not df.empty:
        return df
    dates = pd.date_range(end=datetime.now(), periods=100, freq="H")
    close = np.cumsum(np.random.normal(0, 1, len(dates))) + 100
    return pd.DataFrame({
        "Open": close + np.random.normal(0, 0.2, len(dates)),
...
def render_bot_card(bot_id, bot):
    status_class = {
        ...
        "stopped": "bot-stopped",
        "paused": "bot-paused",
    }.get(getattr(bot, "status", "stopped"), "bot-stopped")

    status_dot = {
        "running": "status-dot-green",
        "stopped": "status-dot-red",
        "paused": "status-dot-yellow",
    }.get(getattr(bot, "status", "stopped"), "status-dot-red")
...
with tab1:
    ...
    if not st.session_state["bots"]:
        st.info("Aucun bot créé pour le moment.")
    else:
        cols = st.columns(2)
...
with tab4:
    ...
    if active_bot and active_bot in st.session_state["logs"]:
        logs = st.session_state["logs"][active_bot]
    else:
        logs = ["[INFO] No logs yet"]
